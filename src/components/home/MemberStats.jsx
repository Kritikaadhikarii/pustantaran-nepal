...
const MemberStats = () => {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      const currentRemaining = Math.round(progress * (targetMembers - memberCount - 11));
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-lg w-full max-w-4xl h-[80vh] relative">
          ...
          <iframe src={url} className="w-full h-full rounded-lg" title="Membership Form" />
        </div>
      </div>
    );
  };
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-lg w-full max-w-2xl h-[600px] my-8 relative flex flex-col">
          ...
          <div className="overflow-y-auto flex-1 p-6">
            <h2 className="text-2xl font-bold text-teal-600 mb-4">Required Information for Membership</h2>
            <p className="text-gray-600 mb-4">Please note that you'll need to provide the following information:</p>
            ...
            <div className="bg-gray-50 p-4 rounded-lg mb-6">
              <p className="font-semibold text-gray-800 mb-2">Membership Fee: NPR 10,000</p>
              <p className="text-gray-600 text-sm">Payment can be made via QR code provided in the form.</p>
            </div>

            <div className="bg-yellow-50 p-4 rounded-lg mb-6">
              <p className="text-yellow-800">
                <span className="font-semibold">Important:</span> All fields in the form are mandatory and must be filled before submission.
              </p>
            </div>
            ...
            <p className="text-gray-700 mb-6">
              ...
              <ul className="mt-2 space-y-2">
                ...
                <li>• <Link to="/products-services" className="text-teal-500 hover:underline">Utilizing our products and services</Link></li>
              </ul>
            </p>
          </div>
          ...
          <div className="border-t p-4 bg-white rounded-b-lg">
            <div className="flex justify-end gap-4">
              ...
              <button onClick={onProceed} className="px-6 py-2 bg-teal-500 text-white rounded-lg hover:bg-teal-600 transition-colors">
                Proceed to Form
              </button>
            </div>
          </div>
      ...
      <div className="text-center space-y-4">
        ...
        <div className="flex justify-center items-center space-x-12 my-8">
          <div className="text-center">
            <div className="text-5xl font-bold text-teal-600 mb-2">{displayCount}</div>
            <div className="text-gray-600">Current Members</div>
          </div>
          <div className="text-center">
            <div className="text-5xl font-bold text-teal-600 mb-2">{displayRemaining.toLocaleString()}</div>
        ...
        <p className="text-lg text-gray-700">
          ...
          <span className="font-bold text-teal-600">{remainingSpots.toLocaleString()}</span> more 
          passionate individuals to reach our goal of{' '}
          <span className="font-bold text-teal-600">{targetMembers.toLocaleString()}</span> members.
        </p>

        <div className="bg-teal-50 p-4 rounded-lg mt-6">
          <p className="text-teal-800">
            Be one of the first {targetMembers.toLocaleString()} members to shape the future of senior care in Nepal!
          </p>
        </div>
